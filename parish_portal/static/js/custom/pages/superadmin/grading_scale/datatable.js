...
var DatatablesServerSide = function() {
  ...
  var initDatatable = function() {
      ...
      dt = $("#gradingscale-datatable").DataTable({
          "paging": true,
          "pagingType": "simple",
          "pageLength": 5,
          ...
          "initComplete": function() {
              ...
              $('#gradingscale-datatable_wrapper .dt-search').addClass('hidden');
          },
          searchDelay: 500,
          serverSide: true,
          responsive: true,
          ...
          columnDefs: [
              ...
              },
              {
                  ...
                  orderable: false,
                  render: function(data) {
                      return `<div class="font-normal whitespace-normal text-sm text-zinc-800 line-clamp-2">${data}</div>`;
                  }
              },
              {
                targets: 3,
          ...
          drawCallback: function() {
              ...
              $('#gradingscale-datatable_length').addClass('flex items-center space-x-2 text-sm bg-white');
              $('#gradingscale-datatable_length select').addClass('px-3 py-1 bg-white border border-gray-300 text-gray-700 text-sm rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500');
              $('#gradingscale-datatable_wrapper .bottom').addClass('flex justify-end items-center gap-x-9 py-6');
              $('#gradingscale-datatable_length label').addClass('text-zinc-950 font-normal text-sm');
              $('#gradingscale-datatable_paginate .page-link').addClass('text-zinc-500 text-sm');
              $('#gradingscale-datatable td').removeClass('p-3 text-end').addClass('p-4 border-b');
              $('#gradingscale-datatable th').removeClass('text-end');
          }
      });
      ...
      $('#gradingscale-datatable tbody').on('change', 'input[type="checkbox"]', function() {
          if (!this.checked) {
  ...
  var updateSelectedCount = function() {
      var selectedCount = $('#gradingscale-datatable tbody input[type="checkbox"]:checked').length;
    ...
    deleteButtons.forEach(d => {
        d.addEventListener('click', function(e) {
            ...
            Modal.showDeleteModal(
              ...
              "Are you sure you want to delete the Grading Scale? This process cannot be undone."
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        success: function(data) {
                          if (data.status_code == 200) {
                            ...
                            Toast.showSuccessToast(
                              'The grading scale was successfully deleted! '
                            )
                          }
                          else {
                        ...
                        error: function() {
                          Toast.showErrorToast(
                            'An error occurred while deleting the grading scale. Please try again.'
                          )
                        }
                    });
