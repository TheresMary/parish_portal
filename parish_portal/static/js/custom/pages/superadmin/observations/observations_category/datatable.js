...
var DatatablesServerSide = function() {
  ...
  var initDatatable = function() {
      ...
      dt = $("#observation_category-datatable").DataTable({
          "paging": true,
          "pagingType": "simple",
          "pageLength": 5,
          ...
          "initComplete": function() {
              ...
              $('#observation_category-datatable_wrapper .dt-search').addClass('hidden');
          },
          searchDelay: 500,
          serverSide: true,
          responsive: true,
                  ...
                  render: function(data, type, row) {
                      ...
                      return `
                        <div class="flex gap-x-5">
                          ...
                          <a href="javascript:void(0)" data-id=${row.id} data-observation-category-table-filter="delete_row">
          ...
          drawCallback: function() {
              ...
              $('#observation_category-datatable_length').addClass('flex items-center space-x-2 text-sm bg-white');
              $('#observation_category-datatable_length select').addClass('px-3 py-1 bg-white border border-gray-300 text-gray-700 text-sm rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500');
              $('#observation_category-datatable_wrapper .bottom').addClass('flex justify-end items-center gap-x-9 py-6');
              $('#observation_category-datatable_length label').addClass('text-zinc-950 font-normal text-sm');
              $('#observation_category-datatable_paginate .page-link').addClass('text-zinc-500 text-sm');
              $('#observation_category-datatable td').removeClass('p-3 text-end').addClass('p-4 border-b');
              $('#observation_category-datatable th').removeClass('text-end');
          }
      });
      ...
      $('#observation_category-datatable tbody').on('change', 'input[type="checkbox"]', function() {
          if (!this.checked) {
  ...
  var updateSelectedCount = function() {
      var selectedCount = $('#observation_category-datatable tbody input[type="checkbox"]:checked').length;
      $('#selectedCount').text(selectedCount + ' row(s) selected');
  };


  var handleDeleteRows = () => {
    const deleteButtons = document.querySelectorAll('[data-observation-category-table-filter="delete_row"]');
    deleteButtons.forEach(d => {
        d.addEventListener('click', function(e) {
            ...
            Modal.showDeleteModal(
              ...
              "Are you sure you want to delete the Observation Category? This process cannot be undone."
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        success: function(data) {
                          if (data.status_code == 200) {
                            ...
                            Toast.showSuccessToast(
                              'The observation category was successfully deleted! '
                            )
                          }
                          else {
                        ...
                        error: function() {
                          Toast.showErrorToast(
                            'An error occurred while deleting the observation category. Please try again.'
                          )
                        }
                    });
