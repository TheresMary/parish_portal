...
var DatatablesServerSide = function() {
  ...
  var initDatatable = function() {
      ...
      dt = $("#competency-datatable").DataTable({
          "paging": true,
          "pagingType": "simple",
          "pageLength": 5,
          ...
          "initComplete": function() {
              ...
              $('#competency-datatable_wrapper .dt-search').addClass('hidden');
          },
          searchDelay: 500,
          serverSide: true,
          responsive: true,
          ...
          drawCallback: function() {
              ...
              $('#competency-datatable_length').addClass('flex items-center space-x-2 text-sm bg-white');
              $('#competency-datatable_length select').addClass('px-3 py-1 bg-white border border-gray-300 text-gray-700 text-sm rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500');
              $('#competency-datatable_wrapper .bottom').addClass('flex justify-end items-center gap-x-9 py-6');
              $('#competency-datatable_length label').addClass('text-zinc-950 font-normal text-sm');
              $('#competency-datatable_paginate .page-link').addClass('text-zinc-500 text-sm');
              $('#competency-datatable td').removeClass('p-3 text-end').addClass('p-4 border-b');
              $('#competency-datatable th').removeClass('text-end');
          }
      });
      ...
      $('#competency-datatable tbody').on('change', 'input[type="checkbox"]', function() {
          if (!this.checked) {
  ...
  var updateSelectedCount = function() {
      var selectedCount = $('#competency-datatable tbody input[type="checkbox"]:checked').length;
    ...
    deleteButtons.forEach(d => {
        d.addEventListener('click', function(e) {
            ...
            Modal.showDeleteModal(
              "Delete Competency",
              "Are you sure you want to delete the Competency? This process cannot be undone."
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        success: function(data) {
                          if (data.status_code == 200) {
                            ...
                            Toast.showSuccessToast(
                              'The competency was successfully deleted! '
                            )
                          }
                          else {
                        ...
                        error: function() {
                          Toast.showErrorToast(
                            'An error occurred while deleting the competency. Please try again.'
                          )
                        }
                    });
