...
var DatatablesServerSide = function() {
  ...
  var initDatatable = function() {
      ...
      dt = $("#officelocation-datatable").DataTable({
          "paging": true,
          "pagingType": "simple",
          "pageLength": 5,
          ...
          "initComplete": function() {
              ...
              $('#officeLocation-datatable_wrapper .dt-search').addClass('hidden');
          },
          searchDelay: 500,
          serverSide: true,
          responsive: true,
          ...
          columnDefs: [
              ...
              },
              {
                ...
                render: function(data, type, row) {
                    ...
                    let view_url = api_config.view_profile.replace('0', row.encrypt_id.toString()) + '?organization_id=' + api_config.org_id;
                  ...
                  render: function(data, type, row) {
                      ...
                      return `
                        <div class="flex gap-x-5">
                          ...
                          <a href="javascript:void(0)" data-id=${row.id} data-officelocation-table-filter="delete_row">
          ...
          drawCallback: function() {
              ...
              $('#officeLocation-datatable_length').addClass('flex items-center space-x-2 text-sm bg-white');
              $('#officeLocation-datatable_length select').addClass('px-3 py-1 bg-white border border-gray-300 text-gray-700 text-sm rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500');
              $('#officeLocation-datatable_wrapper .bottom').addClass('flex justify-end items-center gap-x-9 py-6');
              $('#officeLocation-datatable_length label').addClass('text-zinc-950 font-normal text-sm');
              $('#officeLocation-datatable_paginate .page-link').addClass('text-zinc-500 text-sm');
              $('#officeLocation-datatable td').removeClass('p-3 text-end').addClass('p-4 border-b');
              $('#officeLocation-datatable th').removeClass('text-end');
          }
      });
      ...
      $('#officeLocation-datatable tbody').on('change', 'input[type="checkbox"]', function() {
          if (!this.checked) {
  ...
  var updateSelectedCount = function() {
      var selectedCount = $('#officelocation-datatable tbody input[type="checkbox"]:checked').length;
      $('#selectedCount').text(selectedCount + ' row(s) selected');
  };


  var handleDeleteRows = () => {
    const deleteButtons = document.querySelectorAll('[data-officelocation-table-filter="delete_row"]');
    deleteButtons.forEach(d => {
        d.addEventListener('click', function(e) {
            ...
            Modal.showDeleteModal(
              ...
              "Are you sure you want to delete the Office Location? This process cannot be undone."
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        success: function(data) {
                          if (data.status_code == 200) {
                            ...
                            Toast.showSuccessToast(
                              'The office location was successfully deleted! '
                            )
                          }
                          else {
                        ...
                        error: function() {
                          Toast.showErrorToast(
                            'An error occurred while deleting the office location. Please try again.'
                          )
                        }
                    });
