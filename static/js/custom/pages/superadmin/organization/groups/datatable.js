...
var DatatablesServerSide = function() {
  ...
  var initDatatable = function() {
      ...
      dt = $("#groups-datatable").DataTable({
          ...
          "initComplete": function() {
              ...
              $('#groups-datatable_wrapper .dt-search').addClass('hidden');
          },
          searchDelay: 500,
          serverSide: true,
          responsive: true,
          ...
          columnDefs: [
              ...
              },
              {
                ...
                render: function(data, type, row) {
                    ...
                    if (api_config.from_org === true) {
                        view_url = api_config.group_view_profile.replace('0', row.encrypt_id.toString()) + '?organization_id=' + api_config.org_id;
                      }
                    else {
                        view_url = api_config.group_view_profile.replace('0', row.encrypt_id.toString());
                      }
              ...
              },
              {
                  ...
                  orderable: false,
                  sortable: true,
                  visible: api_config.from_org ? false : true,
                  searchable: api_config.from_org ? false : true,
                  render: function(data) {
              ...
              },
              {
                  ...
                  render: function(data, type, row) {
                      ...
                      return `
                          ...
                          <a href="javascript:void(0)" data-id=${row.id} data-groups-table-filter="delete_row">
          ...
          drawCallback: function() {
              ...
              $('#groups-datatable_length').addClass('flex items-center space-x-2 text-sm bg-white');
              $('#groups-datatable_length select').addClass('px-3 py-1 bg-white border border-gray-300 text-gray-700 text-sm rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500');
              $('#groups-datatable_wrapper .bottom').addClass('flex justify-end items-center gap-x-9 py-6');
              $('#groups-datatable_length label').addClass('text-zinc-950 font-normal text-sm');
              $('#groups-datatable_paginate .page-link').addClass('text-zinc-500 text-sm');
              $('#groups-datatable td').removeClass('p-3 text-end').addClass('p-4 border-b');
              $('#groups-datatable th').removeClass('text-end');
          }
      });
      ...
      $('#groups-datatable tbody').on('change', 'input[type="checkbox"]', function() {
          if (!this.checked) {
  ...
  var updateSelectedCount = function() {
      var selectedCount = $('#groups-datatable tbody input[type="checkbox"]:checked').length;
...
// <!-- To Activate or Inactivate the Countries -->
function updateStatus() {

    let selectedRadio = $('input[name="setactive"]:checked').attr('id');

    // Determine the activation status and value based on the selected radio button
    let status = selectedRadio === 'setActive';  // True if 'setActive' is selected, false otherwise
    ...
    // If there are selected checkboxes, trigger an AJAX call
    if (selectedIds.length > 0) {
      $.ajax({

        method: "POST",
                  url:`${api_config.country_status_set}`,
                  data: {
        ...
        success: function(response) {
          // Update the page based on the response (optional)
          if (response.staus_code= 200) {
            updateStatusForSelected(value);
            $('input[name="setactive"]').prop('checked', false);

            // location.reload();
          } else {
            Toast.showInfoToast("Failed to update the status.");
          }
        },
  ...
  document.querySelectorAll('input[name="setactive"]').forEach(function(radioButton) {
