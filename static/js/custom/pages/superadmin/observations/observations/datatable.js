...
var DatatablesServerSide = function() {
  ...
  var initDatatable = function() {
      ...
      dt = $("#observation-datatable").DataTable({
          "paging": true,
          "pagingType": "simple",
          "pageLength": 5,
          ...
          "initComplete": function() {
              ...
              $('#observation-datatable_wrapper .dt-search').addClass('hidden');
          },
          searchDelay: 500,
          serverSide: true,
          responsive: true,
          ...
          drawCallback: function() {
              ...
              $('#observation-datatable_length').addClass('flex items-center space-x-2 text-sm bg-white');
              $('#observation-datatable_length select').addClass('px-3 py-1 bg-white border border-gray-300 text-gray-700 text-sm rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500');
              $('#observation-datatable_wrapper .bottom').addClass('flex justify-end items-center gap-x-9 py-6');
              $('#observation-datatable_length label').addClass('text-zinc-950 font-normal text-sm');
              $('#observation-datatable_paginate .page-link').addClass('text-zinc-500 text-sm');
              $('#observation-datatable td').removeClass('p-3 text-end').addClass('p-4 border-b');
              $('#observation-datatable th').removeClass('text-end');
          }
      });
      ...
      $('#observation-datatable tbody').on('change', 'input[type="checkbox"]', function() {
          if (!this.checked) {
  ...
  var updateSelectedCount = function() {
      var selectedCount = $('#observation-datatable tbody input[type="checkbox"]:checked').length;
    ...
    deleteButtons.forEach(d => {
        d.addEventListener('click', function(e) {
            ...
            Modal.showDeleteModal(
              "Delete Observation",
              "Are you sure you want to delete the Observation? This process cannot be undone."
                ...
                if (result.value) {
                    $.ajax({
                        ...
                        success: function(data) {
                          if (data.status_code == 200) {
                            ...
                            Toast.showSuccessToast(
                              'The observation was successfully deleted! '
                            )
                          }
                          else {
                        ...
                        error: function() {
                          Toast.showErrorToast(
                            'An error occurred while deleting the observation. Please try again.'
                          )
                        }
                    });
